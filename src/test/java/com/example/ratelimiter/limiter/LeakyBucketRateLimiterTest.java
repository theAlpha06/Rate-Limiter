...
class LeakyBucketRateLimiterTest {
    ...
    // Capacity 5 draining over 5 seconds = exactly 1 unit/second, keeping the
    // arithmetic exact.
    ...
    void setUp() {
        ...
        rateLimiter  = new LeakyBucketRateLimiter(new InMemoryRateLimitStore(), timeProvider);
        policy       = new RateLimitPolicy("test", Algorithm.LEAKY_BUCKET, LIMIT, WINDOW_SIZE);
    }
    ...
    @Test
    void shouldFillBucketToCapacityThenRefuse() {
    ...
    @Test
    void shouldLeakOneUnitPerSecond() {
        assertEquals(LIMIT, allowedOutOf(LIMIT, "user1"));       // fill it
        assertFalse(allow("user1"));

        timeProvider.advance(1);
        assertTrue(allow("user1"), "one second should have leaked exactly one unit");
        assertFalse(allow("user1"), "and only one");

        timeProvider.advance(3);
        assertEquals(3, allowedOutOf(5, "user1"));
    }

    @Test
    void shouldNotLeakBelowEmpty() {
        assertEquals(LIMIT, allowedOutOf(LIMIT, "user1"));

        // Idle far longer than it takes to drain — level must floor at zero, not go negative
    ...
    @Test
    void shouldReportRemainingHeadroom() {
    ...
    @Test
    void shouldReportRetryAfterWhenFull() {
        assertEquals(LIMIT, allowedOutOf(LIMIT, "user1"));
        ...
        assertEquals(1, rejected.retryAfterSeconds(), "one unit/second, so one second");
    }

    @Test
    void shouldRequireHeadroomForCostUnits() {
        assertTrue(rateLimiter.tryAcquire("user1", policy, 3).allowed());
        assertFalse(rateLimiter.tryAcquire("user1", policy, 3).allowed(), "only 2 units of headroom");
    ...
    void shouldRejectCostLargerThanCapacityWithoutConsuming() {
        ...
        assertEquals(LIMIT, allowedOutOf(LIMIT, "user1"), "bucket must still be empty");
    }
